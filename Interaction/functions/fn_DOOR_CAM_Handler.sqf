...
_cam 	= player getVariable ["CQB_doorCam",objNull];
_camTarget = _cam getVariable ["CQB_DOORCAM_TARGET",[0,2,0]];

if (isnil "_cam") then {player setVariable ["CQB_mirrorCamOff",true]};
...
if (_mode == "keydown") then
{
	...
	//--- Terminate
	if (_key in _keysBanned) then {player setVariable ["CQB_mirrorCamOff",true]};

	//--- Start NVG
	if (_key in _keyNightVision) then
	{
		playSound "nvSound";
		_NVGstate = !(player getVariable ["CQB_DOORCAM_NVSTATE", false]);
		"uavrtt" setPiPEffect (if (_NVGstate) then {[1]} else {[0]});
		player setVariable ["CQB_DOORCAM_NVSTATE", _NVGstate];
	};
	...
	if (_key in _keyForward) then
	{
		...
		_cam setVariable ["CQB_DOORCAM_TARGET",_camTarget];
		_pos = _cam modelToWorld _camTarget;
		if (((_pos select 2) - ((getpos _cam) select 2)) < 1.5) then
		{
			_cam camSetTarget _pos;
			if (player getVariable ["CQB_lastSoundTime",time] <= time) then {playsound ["CQB_zoom",true];player setVariable ["CQB_lastSoundTime",time+0.11]};
		};
	};
